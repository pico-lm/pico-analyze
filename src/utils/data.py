...
def get_checkpoint_states(
    ...
    stored in the checkpoint folder in the checkpoint folder (generated by pico-train) and has the
    following structure:
...
def _load_training_config(run_path: str) -> dict:
    """
    Loads in the training config from the run path. If using pico-train, the run_config will always be
...
def _download_training_config(repo_id: str, branch: str) -> dict:
    """
    Downloads the training config from the HuggingFace repository. If using pico-train, the
    run_config will always be stored as training_config.yaml in the root of the repository.
    """
...
def _get_learning_dynamics_commits(repo_id: str, branch: str, data_split: str) -> dict:
    ...
    # NOTE: this pattern is specific to how pico-train saves the learning dynamics data.
